...
describe('test login', () => {
  it('should have the right redirect url and logined success', async () => {
    ...
    pw.onPageLoaded(async (browser, page) => {
      expect(page.url()).to.equal('http://www.testfreelog.com/login?redirect=http%3A%2F%2Fwww.testfreelog.com%2Fuser%2Faccounts', '跳转登录页成功')
      ...
      expect(page.url()).to.equal('http://www.testfreelog.com/user/accounts', '跳转个人中心成功')
